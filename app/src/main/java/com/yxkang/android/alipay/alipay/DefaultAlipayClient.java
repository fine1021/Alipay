...
import com.yxkang.android.alipay.alipay.internal.parser.json.ObjectJsonParser;
...
/**
 * Created by yexiaokang on 2016/5/3.
 */
class DefaultAlipayClient implements AlipayClient {
    ...
    public <T extends AlipayResponse> Map<String, Object> doPost(AlipayRequest<T> request,
        ...
        DateFormat df = new SimpleDateFormat(AlipayConstants.DATE_TIME_FORMAT, Locale.CHINA);
        ...
        try {
            String sysMustQuery = AlipayWebUtils.buildQuery(requestHolder.getProtocalMustParams(),
                    charset);
            String sysOptQuery = AlipayWebUtils.buildQuery(requestHolder.getProtocalOptParams(), charset);
        ...
        try {
            if (request instanceof AlipayUploadRequest) {
                ...
                rsp = AlipayWebUtils.doPost(urlSb.toString(), appParams, fileParams, charset,
                        connectTimeout, readTimeout);
            } else {
                rsp = AlipayWebUtils.doPost(urlSb.toString(), appParams, charset, connectTimeout,
                        readTimeout);
            }
        } catch (IOException e) {
            throw new AlipayApiException(e);
